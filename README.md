...
和其它所有语句块一样，switch 语句块中的内容缩进 +2 个空格。
...
方法名以 [小骆峰](#骆驼峰式命名) 方式编写。

方法名通常是动词或者动词短语。例如 `sendMessage` 或者 `stop`。

下划线可能为了分离命名上的逻辑组件，而出现在 Junit 测试方法名中，每个组件都以 [小骆峰](#骆驼峰式命名) 方式编写。一个典型的模式是 `<methodUnderTest>_<state>`，例如 `pop_emptyStack`。测试方法的命名没有一种唯一正确的方式。
...
非常量字段名（静态或者其它形式）以 [小骆峰](#骆驼峰式命名) 方式编写。

非常量字段名通常是名词或者名词短语。例如：`computedValues` 或者 `index`。

#### 参数名

参数名以 [小骆峰](#骆驼峰式命名) 方式编写。

public 方法中应该避免使用一个字符的参数名。

#### 局部变量名

局部变量名以 [小骆峰](#骆驼峰式命名) 方式编写。

即使局部变量是 final 和不可变的，局部变量也不被认为是常量，并且不应该以常量的风格命名。
...
- 以类的形式命名（请见 [类名](#类名) 章节），跟随着大写字母 T（例如：`RequestT`，`FooBarT` ）。
